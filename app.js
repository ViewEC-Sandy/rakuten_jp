...
function setupTableFilters(){document.querySelectorAll('.table table:not([data-no-auto-filter])').forEach((table,i)=>{if(table.dataset.filterReady)return;table.dataset.filterReady='1';const headers=[...table.querySelectorAll('thead th')].map((th,j)=>({j,label:th.textContent.trim()}));const box=document.createElement('div');box.className='subfilters auto-table-filter';box.innerHTML=`<label>搜尋<input data-role="search" placeholder="搜尋表格內容"></label><label>排序欄位<select data-role="column">${headers.map(h=>`<option value="${h.j}">${esc(h.label)}</option>`).join('')}</select></label><label>順序<select data-role="order"><option value="desc">由大到小</option><option value="asc">由小到大</option></select></label>`;table.closest('.table').before(box);const apply=()=>{const q=box.querySelector('[data-role=search]').value.toLowerCase(),col=+box.querySelector('[data-role=column]').value,dir=box.querySelector('[data-role=order]').value==='asc'?1:-1,tbody=table.tBodies[0],rows=[...tbody.rows];rows.forEach(r=>r.hidden=q&&!r.textContent.toLowerCase().includes(q));rows.sort((a,b)=>compareCell(a.cells[col]?.textContent,b.cells[col]?.textContent)*dir).forEach(r=>tbody.appendChild(r))};box.addEventListener('input',apply);box.addEventListener('change',apply);new MutationObserver(apply).observe(table.tBodies[0],{childList:true})})}
