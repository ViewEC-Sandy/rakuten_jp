...
function resolveProductReference(value){const key=String(value||'').trim(),exact=findProduct(key);if(exact)return{product:exact,managementNumber:exact.managementNumber||'',nameZh:exact.nameZh||exact.name||'',nameJa:exact.nameJa||'',projectName:exact.projectName||'',matchedBy:'productNumber'};const matches=findProductsByManagementNumber(key);if(!matches.length)return{product:null,managementNumber:key,nameZh:'',nameJa:'',projectName:'',matchedBy:''};const names=[...new Set(matches.map(x=>String(x.nameZh||x.name||'').trim()).filter(Boolean))],namesJa=[...new Set(matches.map(x=>String(x.nameJa||'').trim()).filter(Boolean))],projects=[...new Set(matches.map(x=>String(x.projectName||'').trim()).filter(Boolean))];return{product:matches[0],managementNumber:matches[0].managementNumber||key,nameZh:names.join('／'),nameJa:namesJa.join('／'),projectName:projects.join('／'),matchedBy:'managementNumber',matches}}
function resolveAdProductReference(row){const byProduct=resolveProductReference(row?.productId||'');if(byProduct.product)return byProduct;const byManagement=resolveProductReference(row?.managementNumber||'');if(byManagement.product)return byManagement;return{product:null,managementNumber:String(row?.managementNumber||row?.productId||'').trim(),nameZh:'',nameJa:'',projectName:'',matchedBy:''}}
...
function renderTopProducts(){const m=new Map(),orderRevenue=new Map(canonicalOrderRevenueRows(state.sales).map(r=>[orderKey(r),num(r.revenue)])),usedOrders=new Set();state.sales.forEach(r=>{const ref=resolveSalesProduct(r),p=ref.product||{},k=p.id||r.productId||r.managementNumber,name=ref.nameZh||p.nameZh||p.name||p.nameJa||'未設定';if(!m.has(k))m.set(k,{productId:k,name,revenue:0,quantity:0});const x=m.get(k);x.name=name;x.quantity+=num(r.quantity);const ok=orderKey(r);if(!usedOrders.has(ok)){x.revenue+=orderRevenue.get(ok)||0;usedOrders.add(ok)}});const rows=[...m.values()].sort((a,b)=>b.revenue-a.revenue).slice(0,10);$('topProductRows').innerHTML=rows.map((x,i)=>`<tr><td>${i+1}</td><td>${esc(x.productId)}</td><td>${esc(x.name)}</td><td>${yen(x.revenue)}</td><td>${fmt(x.quantity)}</td></tr>`).join('')}
