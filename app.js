...
$('costLoadBtn').onclick=loadCostProductFromInput;$('costSaveBtn').onclick=saveCurrentProductCost;['costSupply','costLength','costWidth','costHeight','costWeight','costMultiplier','costNote'].forEach(id=>$(id)?.addEventListener('input',renderCostPricing));$('costNewBtn').onclick=()=>$('costNewModal').classList.remove('hidden');$('newCostCancelBtn').onclick=()=>$('costNewModal').classList.add('hidden');$('newCostCreateBtn').onclick=createCostProduct;$('costImportBtn').onclick=()=>$('costImportFile').click();$('costImportFile').onchange=e=>e.target.files?.[0]&&importCostFile(e.target.files[0]);
...
const DEFAULT_CALC_PARAMS={multiplier:1.3,fxJpyPerTwd:4.5,targetMargin:10,warehouse:75,volDivisor:6000,epacketMaxKg:2,epacketMaxSide:60,epacketMaxSum:90,fees:{Rakuten:10,NETSEA:4,Shopify:3.4,TikTok:10}};
...
function productPlatformCalc(p,platform){const price=productPlatformPrice(p,platform),fee=price*platformFeeRate(platform),twdRevenue=calcParams.fxJpyPerTwd?price/calcParams.fxJpyPerTwd:0,cost=num(p.supplyPrice),profit=twdRevenue-(calcParams.fxJpyPerTwd?fee/calcParams.fxJpyPerTwd:0)-cost;return{price,fee,twdRevenue,profit,margin:twdRevenue?profit/twdRevenue*100:0}}
...
function costForProduct(p){const saved=loadCostStore()[p?.id]||{};const legacyKg=saved.weightKg===undefined&&saved.weightG!==undefined?num(saved.weightG)/1000:0;return{supply:saved.supply??num(p?.supplyPrice),length:saved.length??0,width:saved.width??0,height:saved.height??0,weightKg:saved.weightKg??legacyKg,multiplier:saved.multiplier??'',note:saved.note??''}}
function round2(v){return Math.round((num(v)+Number.EPSILON)*100)/100}
function shippingCalc(c){const realKg=round2(num(c.weightKg)),volKg=round2(num(c.length)*num(c.width)*num(c.height)/(num(calcParams.volDivisor)||6000));const epacket=realKg<=num(calcParams.epacketMaxKg)&&Math.max(num(c.length),num(c.width),num(c.height))<=num(calcParams.epacketMaxSide)&&(num(c.length)+num(c.width)+num(c.height))<=num(calcParams.epacketMaxSum);const method=epacket?'E小包':'EMS';const chargeKg=method==='E小包'?realKg:Math.max(realKg,volKg);let freight=0;if(method==='E小包'){const step=Math.max(.1,Math.ceil((realKg-1e-9)*10)/10);freight=step<=2?100+Math.max(0,Math.round((step-.1)*10))*20:0}else{const step=Math.max(.1,Math.ceil((chargeKg-1e-9)*10)/10);if(step<=2)freight=289+Math.max(0,Math.round((step-.1)*10))*9;else if(step<=4)freight=540+Math.max(0,Math.round((step-2.1)*10))*10;else freight=730+Math.ceil((step-4)*10-1e-9)*9}const warehouse=num(calcParams.warehouse),logistics=freight+warehouse;return{realKg,volKg,chargeKg:round2(chargeKg),method,freight,warehouse,logistics}}
function pricingCalc(c,platform='Rakuten'){const ship=shippingCalc(c),mult=num(c.multiplier)||num(calcParams.multiplier),fx=num(calcParams.fxJpyPerTwd),rate=platformFeeRate(platform),formulaPrice=(num(c.supply)*mult+ship.logistics)*fx,revenue=formulaPrice/fx,fee=formulaPrice*rate,profit=(formulaPrice-fee)/fx-num(c.supply)-ship.logistics,margin=revenue?profit/revenue*100:0,target=num(calcParams.targetMargin)/100,den=1-rate-target,targetPrice=den>0?((num(c.supply)+ship.logistics)*fx/den):formulaPrice;return{...ship,mult,formulaPrice,fee,revenue,profit,margin,targetPrice:Math.ceil(targetPrice)}}
...
function openProductCost(sku,navigate=true){const p=findProduct(sku);if(!p)return;currentCostSku=p.id;if(navigate)show('costPricing');$('costProductSearch').value=p.id;const c=costForProduct(p);$('costSupply').value=c.supply;$('costLength').value=c.length;$('costWidth').value=c.width;$('costHeight').value=c.height;$('costWeight').value=c.weightKg;$('costMultiplier').value=c.multiplier;$('costNote').value=c.note;$('costProductSummary').innerHTML=`<strong>${esc(p.id)}</strong>　${esc(p.managementNumber||'')}　${esc(p.nameZh||'')}　<span class="muted">${esc(p.projectName||'')}</span>`;renderCostPricing()}
function currentCostValues(){return{supply:num($('costSupply').value),length:num($('costLength').value),width:num($('costWidth').value),height:num($('costHeight').value),weightKg:num($('costWeight').value),multiplier:$('costMultiplier').value===''?'':num($('costMultiplier').value),note:$('costNote').value||''}}
function renderCostPricing(){if(!currentCostSku)return;const p=findProduct(currentCostSku);if(!p)return;const c=currentCostValues(),x=pricingCalc(c,'Rakuten');$('costTotal').textContent=twd(x.logistics);$('costFormulaBreakdown').innerHTML=[['實重',x.realKg.toFixed(2)+' kg'],['材積重',x.volKg.toFixed(2)+' kg'],['EMS 計費重',Math.max(x.realKg,x.volKg).toFixed(2)+' kg'],['配送方式',x.method],['實際運費計費依據',x.method==='E小包'?x.realKg.toFixed(2)+' kg 實重':x.chargeKg.toFixed(2)+' kg 計費重'],['運費',twd(x.freight)],['平均進出倉',twd(x.warehouse)]].map(([n,v])=>`<div><span>${n}</span><strong>${v}</strong></div>`).join('');$('costFormulaPrice').textContent=yen(x.formulaPrice);$('costRakutenMargin').textContent=x.margin.toFixed(1)+'%';$('costRakutenMargin').className=x.margin<num(calcParams.targetMargin)?'neg':'pos';$('costTargetPrice').textContent=yen(Math.max(x.formulaPrice,x.targetPrice));$('costPlatformRows').innerHTML=['Rakuten','NETSEA','Shopify','TikTok'].map(platform=>{const z=pricingCalc(c,platform),rate=platformFeeRate(platform);return`<tr><td>${platform}</td><td>${yen(z.formulaPrice)}</td><td>${(rate*100).toFixed(1)}%</td><td>${yen(z.fee)}</td><td>${twd(z.revenue)}</td><td>${twd(c.supply)}</td><td>${twd(z.logistics)}</td><td>${twd(z.profit)}</td><td class="${z.margin<num(calcParams.targetMargin)?'neg':'pos'}">${z.margin.toFixed(1)}%</td><td>${yen(Math.max(z.formulaPrice,z.targetPrice))}</td></tr>`}).join('')}
...
function renderCostList(){if(!$('costListRows'))return;const store=loadCostStore();$('costListRows').innerHTML=Object.entries(store).map(([sku,c])=>{const p=findProduct(sku)||{id:sku},x=pricingCalc(c,'Rakuten');return`<tr><td><button class="ghost cost-row-open" data-sku="${esc(sku)}">${esc(sku)}</button></td><td>${esc(p.managementNumber||'')}</td><td>${esc(p.nameZh||'')}</td><td>${twd(c.supply)}</td><td>${num(c.length)}</td><td>${num(c.width)}</td><td>${num(c.height)}</td><td>${num(c.weightKg).toFixed(2)}kg</td><td>${x.volKg.toFixed(2)}kg</td><td>${x.method}</td><td>${twd(x.freight)}</td><td>${twd(x.logistics)}</td><td>${yen(x.formulaPrice)}</td><td class="${x.margin<num(calcParams.targetMargin)?'neg':'pos'}">${x.margin.toFixed(1)}%</td><td>${yen(Math.max(x.formulaPrice,x.targetPrice))}</td><td class="cost-note-cell">${esc(c.note||'')}</td></tr>`}).join('');$('costListRows').querySelectorAll('.cost-row-open').forEach(b=>b.onclick=()=>openProductCost(b.dataset.sku,false))}
async function createCostProduct(){if(!['admin','manager'].includes(state.role))return $('newCostStatus').textContent='目前為 Viewer，無法建立商品。';const sku=$('newCostSku').value.trim();if(!sku)return $('newCostStatus').textContent='請輸入商品番号 / SKU';if(findProduct(sku))return $('newCostStatus').textContent='此 SKU 已存在';try{$('newCostStatus').textContent='建立中…';await setDoc(doc(db,'products',sku),{managementNumber:$('newCostManagement').value.trim(),nameZh:$('newCostName').value.trim(),projectName:$('newCostProject').value.trim(),supplyPrice:num($('newCostSupply').value),createdAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:false});const store=loadCostStore();store[sku]={supply:num($('newCostSupply').value),length:num($('newCostLength').value),width:num($('newCostWidth').value),height:num($('newCostHeight').value),weightKg:num($('newCostWeight').value),multiplier:'',note:$('newCostNote').value||''};saveCostStore(store);await loadProductMaster();$('costNewModal').classList.add('hidden');openProductCost(sku,false);renderCostList()}catch(e){$('newCostStatus').textContent='建立失敗：'+e.message}}
...
async function importCostFile(file){
  try{
    ...
    const label={sku:'商品番号',mgmt:'商品管理番号',supply:'供應價',length:'長',width:'寬',height:'高',weightKg:'實重(kg)',multiplier:'倍率',note:'備註'};
    ...
    if(!updateFields.length)throw new Error('沒有辨識到可更新欄位（供應價、長、寬、高、實重、倍率、備註）');
    ...
    for(let i=0;i<rows.length;i++){
      ...
      for(const k of updateFields){
        ...
        if(k==='note'){next.note=String(v);changed++;continue}
