...
function openProductCost(sku,navigate=true){const p=findProduct(sku);if(!p)return;currentCostSku=p.id;if(navigate)show('costPricing');$('costProductSearch').value=p.id;const c=costForProduct(p);$('costSupply').value=c.supply;$('costLength').value=c.length;$('costWidth').value=c.width;$('costHeight').value=c.height;$('costWeight').value=c.weightG;$('costMultiplier').value=c.multiplier;$('costNote').value=c.note;$('costProductSummary').innerHTML=`<strong>${esc(p.id)}</strong>　${esc(p.managementNumber||'')}　${esc(p.nameZh||'')}　<span class="muted">${esc(p.projectName||'')}</span>`;renderCostPricing()}
function currentCostValues(){return{supply:num($('costSupply').value),length:num($('costLength').value),width:num($('costWidth').value),height:num($('costHeight').value),weightG:num($('costWeight').value),multiplier:$('costMultiplier').value===''?'':num($('costMultiplier').value),note:$('costNote').value||''}}
...
function renderCostList(){if(!$('costListRows'))return;const store=loadCostStore();$('costListRows').innerHTML=Object.entries(store).map(([sku,c])=>{const p=findProduct(sku)||{id:sku},x=pricingCalc(c,'Rakuten');return`<tr><td><button class="ghost cost-row-open" data-sku="${esc(sku)}">${esc(sku)}</button></td><td>${esc(p.managementNumber||'')}</td><td>${esc(p.nameZh||'')}</td><td>${twd(c.supply)}</td><td>${num(c.length)}</td><td>${num(c.width)}</td><td>${num(c.height)}</td><td>${num(c.weightG)}g</td><td>${x.volKg.toFixed(2)}kg</td><td>${x.method}</td><td>${twd(x.freight)}</td><td>${twd(x.logistics)}</td><td>${yen(x.formulaPrice)}</td><td class="${x.margin<num(calcParams.targetMargin)?'neg':'pos'}">${x.margin.toFixed(1)}%</td><td>${yen(Math.max(x.formulaPrice,x.targetPrice))}</td><td class="cost-note-cell">${esc(c.note||'')}</td></tr>`}).join('');$('costListRows').querySelectorAll('.cost-row-open').forEach(b=>b.onclick=()=>openProductCost(b.dataset.sku,false))}
async function createCostProduct(){if(!['admin','manager'].includes(state.role))return $('newCostStatus').textContent='目前為 Viewer，無法建立商品。';const sku=$('newCostSku').value.trim();if(!sku)return $('newCostStatus').textContent='請輸入商品番号 / SKU';if(findProduct(sku))return $('newCostStatus').textContent='此 SKU 已存在';try{$('newCostStatus').textContent='建立中…';await setDoc(doc(db,'products',sku),{managementNumber:$('newCostManagement').value.trim(),nameZh:$('newCostName').value.trim(),projectName:$('newCostProject').value.trim(),supplyPrice:num($('newCostSupply').value),createdAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:false});const store=loadCostStore();store[sku]={supply:num($('newCostSupply').value),length:num($('newCostLength').value),width:num($('newCostWidth').value),height:num($('newCostHeight').value),weightG:num($('newCostWeight').value),multiplier:'',note:$('newCostNote').value||''};saveCostStore(store);await loadProductMaster();$('costNewModal').classList.add('hidden');openProductCost(sku,false);renderCostList()}catch(e){$('newCostStatus').textContent='建立失敗：'+e.message}}
function pickField(row,names){for(const n of names){if(row[n]!==undefined&&String(row[n]).trim()!=='')return row[n]}return''}
async function importCostFile(file){try{$('costImportStatus').textContent='讀取中…';let rows=[];if(/\.xlsx?$/i.test(file.name)){const data=await file.arrayBuffer(),wb=XLSX.read(data,{type:'array'}),ws=wb.Sheets[wb.SheetNames[0]];rows=XLSX.utils.sheet_to_json(ws,{defval:''})}else rows=await new Promise((res,rej)=>Papa.parse(file,{header:true,skipEmptyLines:'greedy',complete:r=>res(r.data),error:rej}));const store=loadCostStore();let ok=0,skip=0;for(const row of rows){const sku=String(pickField(row,['商品番号','SKU','SKU管理番号','商品規格管理編號'])).trim(),mgmt=String(pickField(row,['商品管理番号','GOT&NETSEA 商品管理番号','商品管理編號'])).trim();const p=findProduct(sku)||findProductByManagementNumber(mgmt);if(!p){skip++;continue}const old=store[p.id]||costForProduct(p),next={...old};const map=[['supply',['商品供應價(TWD)','商品供應價','台幣供貨價 (含稅)NT$','台幣供貨價']],['length',['外箱長(cm)','長度 (cm)','長度','長']],['width',['外箱寬(cm)','寬度 (cm)','寬度','寬']],['height',['外箱高(cm)','高度 (cm)','高度','高']],['weightG',['商品重量(g)','商品實重(g)','毛重 (g)','毛重','重量(g)']],['multiplier',['倍率']],['note',['備註']]];for(const [k,names] of map){const v=pickField(row,names);if(v!==''&&v!==null&&v!==undefined)next[k]=k==='note'?String(v):num(v)}store[p.id]=next;ok++}saveCostStore(store);$('costImportStatus').textContent=`完成：更新 ${ok} 筆，找不到商品跳過 ${skip} 筆。空白欄位不覆蓋既有資料。`;renderCostList();if(currentCostSku)openProductCost(currentCostSku,false)}catch(e){$('costImportStatus').textContent='匯入失敗：'+e.message}}
