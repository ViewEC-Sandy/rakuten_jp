...
function findProductsByManagementNumber(value){return state.productsByManagement.get(String(value||'').trim())||[]}
function findProductByManagementNumber(value){return findProductsByManagementNumber(value)[0]||null}
function resolveProductReference(value){const key=String(value||'').trim(),exact=findProduct(key);if(exact)return{product:exact,managementNumber:exact.managementNumber||'',nameZh:exact.nameZh||exact.name||'',projectName:exact.projectName||'',matchedBy:'productNumber'};const matches=findProductsByManagementNumber(key);if(!matches.length)return{product:null,managementNumber:'',nameZh:'',projectName:'',matchedBy:''};const names=[...new Set(matches.map(x=>String(x.nameZh||x.name||'').trim()).filter(Boolean))],projects=[...new Set(matches.map(x=>String(x.projectName||'').trim()).filter(Boolean))];return{product:matches[0],managementNumber:key,nameZh:names.join('／'),projectName:projects.join('／'),matchedBy:'managementNumber',matches}}
function resolveSalesProduct(row){const byProduct=resolveProductReference(row?.productId);if(byProduct.product)return byProduct;const byManagement=resolveProductReference(row?.managementNumber);if(byManagement.product)return byManagement;return byProduct}
function dedupeSalesRows(rows){const seen=new Set(),out=[];for(const r of rows){const d=r.saleDate?.toDate?r.saleDate.toDate():new Date(r.saleDate);const key=[canonicalPlatform(r.platform),String(r.orderId||''),String(r.productId||''),String(r.managementNumber||''),fdLocal(d),num(r.quantity),num(r.revenue)].join('||');if(seen.has(key))continue;seen.add(key);out.push(r)}return out}
function uniqueOrderRevenue(rows){const orders=new Map();for(const r of rows){const key=canonicalPlatform(r.platform)+'||'+String(r.orderId||'');const v=num(r.revenue);orders.set(key,Math.max(orders.get(key)||0,v))}return sum([...orders.values()])}
...
function renderTrend(){const granularity=$('granularity')?.value||'month',a=by(state.sales,r=>periodKey(r,granularity),r=>r.revenue),b=by(state.compare,r=>periodKey(r,granularity),r=>r.revenue),labels=orderedPeriods(granularity,a,b),currentLabel=periodLabelForSeries(state.sales,'本期'),compareLabel=periodLabelForSeries(state.compare,'比較期');chart('trend','line',labels,[{label:currentLabel,data:labels.map(x=>a[x]??null),spanGaps:true},{label:compareLabel,data:labels.map(x=>b[x]??null),spanGaps:true}],{plugins:{tooltip:{callbacks:{label:ctx=>`${ctx.dataset.label}：${yen(ctx.raw)}`}}},scales:{y:{beginAtZero:true}}})}
...
function renderTopProducts(){const m=new Map();state.sales.forEach(r=>{const ref=resolveSalesProduct(r),p=ref.product||{},k=p.id||r.productId||r.managementNumber;if(!m.has(k))m.set(k,{productId:k,name:ref.nameZh||p.nameZh||p.name||p.nameJa||'未設定',revenue:0,quantity:0});const x=m.get(k);x.revenue+=num(r.revenue);x.quantity+=num(r.quantity)});const rows=[...m.values()].sort((a,b)=>b.revenue-a.revenue).slice(0,10);$('topProductRows').innerHTML=rows.map((x,i)=>`<tr><td>${i+1}</td><td>${esc(x.productId)}</td><td>${esc(x.name)}</td><td>${yen(x.revenue)}</td><td>${fmt(x.quantity)}</td></tr>`).join('')}
...
async function importProductAnalytics(file){
  ...
  try{
    ...
    const rows=[...grouped.values()].map(g=>{const ref=resolveProductReference(g.productId);return{id:safe('product_analysis_'+monthKey+'_'+g.productId),month:monthKey,productId:g.productId,managementNumber:ref.managementNumber||'',...g,sourceFile:file.name}});
