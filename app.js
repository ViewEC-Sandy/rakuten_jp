...
$('masterColumnsBtn').onclick=()=>{const p=$('masterColumnPanel');p.classList.toggle('hidden');if(!p.classList.contains('hidden'))renderMasterColumnPanel()};
$('saveCalcParamsBtn').onclick=saveCalcParams;
$('resetCalcParamsBtn').onclick=()=>{calcParams=structuredClone(DEFAULT_CALC_PARAMS);localStorage.setItem('japanEcCalcParams',JSON.stringify(calcParams));renderCalcParams();saveCalcParams()};
$('addVendorCommissionBtn').onclick=addVendorCommissionSetting;
$('pricingCalcBtn').onclick=renderPricing;$('pricingCost').oninput=renderPricing;
$('costLoadBtn').onclick=loadCostProductFromInput;$('costSaveBtn').onclick=saveCurrentProductCost;['costShippingOrigin','costSupply','costLength','costWidth','costHeight','costWeight','costMultiplier','costNote'].forEach(id=>$(id)?.addEventListener('input',renderCostPricing));$('costNewBtn').onclick=()=>$('costNewModal').classList.remove('hidden');$('newCostCancelBtn').onclick=()=>$('costNewModal').classList.add('hidden');$('newCostCreateBtn').onclick=createCostProduct;$('costImportBtn').onclick=()=>$('costImportFile').click();$('costImportFile').onchange=e=>e.target.files?.[0]&&importCostFile(e.target.files[0]);$('costShippingFilter')?.addEventListener('change',renderCostList);
...
async function loadPlatforms(){state.platforms=new Set(['Rakuten','NETSEA','Shopify','TikTok']);$('platform').innerHTML='<option value="">全部平台</option>'+[...state.platforms].map(p=>`<option value="${esc(p)}">${esc(p)}</option>`).join('')}
...
function canonicalPlatform(value){const v=String(value||'').trim(),k=v.toLowerCase();if(k==='rakuten'||k==='taiwan_rakuten')return'Rakuten';if(k==='netsea')return'NETSEA';if(k==='shopify'||k==='rianyou_shopify')return'Shopify';if(k==='tiktok'||k==='tik tok')return'TikTok';return v}
function isSupportedPlatform(value){return ['Rakuten','NETSEA','Shopify','TikTok'].includes(value)}
...
function renderProducts(){const q=$('productSearch').value.toLowerCase(),projectFilter=$('productProjectFilter').value;const rows=[...state.products.values()].filter(p=>{const project=String(p.projectName||'').trim();return!isHiddenProject(project)&&(!projectFilter||project===projectFilter)&&(!q||((p.id||'')+' '+(p.managementNumber||'')+' '+(p.nameZh||'')+' '+project).toLowerCase().includes(q))});const totals={revenue:uniqueOrderRevenue(state.sales),quantity:sum(state.sales.map(x=>x.quantity))};$('productTotals').innerHTML=[['篩選營收總計',yen(totals.revenue)],['篩選銷量總計',fmt(totals.quantity)],['商品數',fmt(rows.length)]].map(([n,v])=>`<div class="kpi"><span>${n}</span><strong>${v}</strong></div>`).join('');$('productRows').innerHTML=rows.slice(0,500).map(p=>`<tr><td>${esc(p.id)}</td><td>${esc(p.managementNumber||'')}</td><td>${esc(p.nameZh||p.name||'')}</td>${['Rakuten','NETSEA','Shopify','TikTok'].map(platform=>{const c=productPlatformCalc(p,platform);return`<td><strong>${yen(c.price)}</strong><br><small>利潤率 ${c.margin.toFixed(1)}%</small></td>`}).join('')}</tr>`).join('')}
...
const DEFAULT_CALC_PARAMS={multiplier:1.3,fxJpyPerTwd:4.5,targetMargin:10,warehouse:75,volDivisor:6000,epacketMaxKg:2,epacketMaxSide:60,epacketMaxSum:90,vendorDefaultCommission:10,fees:{Rakuten:10,NETSEA:4,Shopify:3.4,TikTok:10}};
let calcParams=loadCalcParams();
function loadCalcParams(){try{return {...DEFAULT_CALC_PARAMS,...JSON.parse(localStorage.getItem('japanEcCalcParams')||'{}'),fees:{...DEFAULT_CALC_PARAMS.fees,...(JSON.parse(localStorage.getItem('japanEcCalcParams')||'{}').fees||{})}}}catch{return structuredClone(DEFAULT_CALC_PARAMS)}}
function platformFeeRate(platform){return num(calcParams.fees?.[platform])/100}
const VENDOR_COMMISSION_KEY='japanEcVendorCommissionsV1193';
function loadVendorCommissions(){try{return JSON.parse(localStorage.getItem(VENDOR_COMMISSION_KEY)||'{}')}catch{return{}}}
function saveVendorCommissions(v){localStorage.setItem(VENDOR_COMMISSION_KEY,JSON.stringify(v))}
function vendorCommissionInfo(p){const vendor=String(p?.vendorName||'').trim(),settings=loadVendorCommissions(),cfg=vendor?settings[vendor]:null;if(!cfg||!cfg.enabled)return{vendor,enabled:false,rate:0,source:'無抽成'};const custom=cfg.rate===''||cfg.rate===null||cfg.rate===undefined?null:num(cfg.rate),rate=(custom===null?num(calcParams.vendorDefaultCommission):custom)/100;return{vendor,enabled:true,rate,source:custom===null?'預設':'個別'}}
function renderVendorCommissionSettings(){const body=$('vendorCommissionRows');if(!body)return;const settings=loadVendorCommissions();const vendors=[...new Set([...state.products.values()].map(p=>String(p.vendorName||'').trim()).filter(Boolean).concat(Object.keys(settings)))].sort((a,b)=>a.localeCompare(b,'zh-Hant'));body.innerHTML=vendors.map(v=>{const cfg=settings[v]||{enabled:false,rate:''},actual=cfg.enabled?((cfg.rate===''||cfg.rate===null||cfg.rate===undefined)?num(calcParams.vendorDefaultCommission):num(cfg.rate)):0;return `<tr data-vendor="${esc(v)}"><td>${esc(v)}</td><td><select class="vendor-enabled"><option value="0" ${!cfg.enabled?'selected':''}>無</option><option value="1" ${cfg.enabled?'selected':''}>有</option></select></td><td><input class="vendor-rate" type="number" min="0" step="0.1" value="${cfg.rate??''}" placeholder="預設 ${num(calcParams.vendorDefaultCommission)}%"></td><td>${actual.toFixed(1)}%</td><td class="editor"><button class="ghost vendor-save">儲存</button><button class="danger vendor-delete">刪除設定</button></td></tr>`}).join('');body.querySelectorAll('.vendor-save').forEach(b=>b.onclick=()=>{const tr=b.closest('tr'),v=tr.dataset.vendor,cfg={enabled:tr.querySelector('.vendor-enabled').value==='1',rate:tr.querySelector('.vendor-rate').value===''?'':num(tr.querySelector('.vendor-rate').value)};const all=loadVendorCommissions();all[v]=cfg;saveVendorCommissions(all);renderVendorCommissionSettings();renderCostList();if(currentCostSku)renderCostPricing()});body.querySelectorAll('.vendor-delete').forEach(b=>b.onclick=()=>{const v=b.closest('tr').dataset.vendor,all=loadVendorCommissions();delete all[v];saveVendorCommissions(all);renderVendorCommissionSettings();renderCostList();if(currentCostSku)renderCostPricing()})}
function addVendorCommissionSetting(){const vendor=prompt('請輸入廠商名（需與商品主檔的廠商名一致）');if(!vendor||!vendor.trim())return;const v=vendor.trim(),all=loadVendorCommissions();if(!all[v])all[v]={enabled:true,rate:''};saveVendorCommissions(all);renderVendorCommissionSettings()}
function productPlatformPrice(p,platform){const keys={Rakuten:'rakutenPriceJpy',NETSEA:'netseaPriceJpy',Shopify:'shopifyPrice',TikTok:'tiktokPriceJpy'};return num(p?.[keys[platform]])}
function productPlatformCalc(p,platform){const price=productPlatformPrice(p,platform),fee=price*platformFeeRate(platform),fx=num(calcParams.fxJpyPerTwd),twdRevenue=fx?price/fx:0,vc=vendorCommissionInfo(p),vendorCommissionJpy=price*vc.rate,vendorCommissionTwd=fx?vendorCommissionJpy/fx:0,actualVendorPayment=num(p.supplyPrice)-vendorCommissionTwd,profit=twdRevenue-(fx?fee/fx:0)-actualVendorPayment;return{price,fee,twdRevenue,profit,margin:twdRevenue?profit/twdRevenue*100:0,vendorCommission:vc,vendorCommissionJpy,vendorCommissionTwd,actualVendorPayment}}
const MASTER_COLUMNS=[
 ['costAction','計算明細','base'],['id','商品番号','base'],['managementNumber','商品管理番号','base'],['nameZh','中文商品名','base'],['projectName','專案名稱','base'],['vendorName','廠商名','base'],['supplyPrice','商品供應價(TWD)','cost'],['barcode','商品條碼','base'],
 ['Rakuten_price','Rakuten 售價(JPY)','Rakuten'],['Rakuten_fee','Rakuten 平台費(JPY)','Rakuten'],['Rakuten_profit','Rakuten 利潤(TWD)','Rakuten'],['Rakuten_margin','Rakuten 利潤率','Rakuten'],
 ['NETSEA_price','NETSEA 售價(JPY)','NETSEA'],['NETSEA_fee','NETSEA 平台費(JPY)','NETSEA'],['NETSEA_profit','NETSEA 利潤(TWD)','NETSEA'],['NETSEA_margin','NETSEA 利潤率','NETSEA'],
 ['Shopify_price','Shopify 售價(JPY)','Shopify'],['Shopify_fee','Shopify 平台費(JPY)','Shopify'],['Shopify_profit','Shopify 利潤(TWD)','Shopify'],['Shopify_margin','Shopify 利潤率','Shopify'],
 ['TikTok_price','TikTok 售價(JPY)','TikTok'],['TikTok_fee','TikTok 平台費(JPY)','TikTok'],['TikTok_profit','TikTok 利潤(TWD)','TikTok'],['TikTok_margin','TikTok 利潤率','TikTok']
];
const DEFAULT_MASTER_COLS=['costAction','id','managementNumber','nameZh','supplyPrice','Rakuten_price','Rakuten_margin','NETSEA_price','NETSEA_margin','Shopify_price','Shopify_margin','TikTok_price','TikTok_margin'];
function selectedMasterColumns(){try{return JSON.parse(localStorage.getItem('masterVisibleColumns')||'null')||DEFAULT_MASTER_COLS}catch{return DEFAULT_MASTER_COLS}}
function setMasterColumns(cols){localStorage.setItem('masterVisibleColumns',JSON.stringify(cols));renderMaster()}
function renderMasterColumnPanel(){const box=$('masterColumnPanel');if(!box)return;const selected=new Set(selectedMasterColumns()),groups=['base','cost','Rakuten','NETSEA','Shopify','TikTok'];box.innerHTML='<div class="column-actions"><button type="button" data-cols="all" class="ghost">全選</button><button type="button" data-cols="default" class="ghost">恢復預設</button></div>'+groups.map(g=>`<fieldset><legend>${g==='base'?'基本資料':g==='cost'?'成本':g}</legend>${MASTER_COLUMNS.filter(x=>x[2]===g).map(([k,l])=>`<label><input type="checkbox" value="${k}" ${selected.has(k)?'checked':''}> ${l}</label>`).join('')}</fieldset>`).join('');box.querySelectorAll('input').forEach(x=>x.onchange=()=>setMasterColumns([...box.querySelectorAll('input:checked')].map(i=>i.value)));box.querySelector('[data-cols="all"]').onclick=()=>setMasterColumns(MASTER_COLUMNS.map(x=>x[0]));box.querySelector('[data-cols="default"]').onclick=()=>setMasterColumns(DEFAULT_MASTER_COLS)}
function masterCell(p,key){if(key==='costAction')return `<button class="ghost cost-detail-btn" data-sku="${esc(p.id)}">查看計算</button>`;if(['id','managementNumber','nameZh','projectName','vendorName','barcode'].includes(key))return esc(p[key]||'');if(key==='supplyPrice')return twd(p.supplyPrice);const [platform,metric]=key.split('_'),c=productPlatformCalc(p,platform);if(metric==='price'||metric==='fee')return yen(c[metric]);if(metric==='profit')return twd(c.profit);if(metric==='margin')return `<span class="${c.margin<10?'neg':''}">${c.margin.toFixed(1)}%</span>`;return''}
function renderMaster(){const q=$('masterSearch').value.trim().toLowerCase(),cols=selectedMasterColumns();$('masterHead').innerHTML='<tr>'+cols.map(k=>`<th>${esc(MASTER_COLUMNS.find(x=>x[0]===k)?.[1]||k)}</th>`).join('')+'</tr>';$('masterRows').innerHTML=[...state.products.values()].filter(p=>!q||((p.id||'')+' '+(p.managementNumber||'')+' '+(p.nameZh||'')+' '+(p.projectName||'')+' '+(p.vendorName||'')+' '+(p.barcode||'')).toLowerCase().includes(q)).sort((a,b)=>String(a.id).localeCompare(String(b.id),'ja')).map(p=>'<tr>'+cols.map(k=>`<td>${masterCell(p,k)}</td>`).join('')+'</tr>').join('');$('masterRows').querySelectorAll('.cost-detail-btn').forEach(b=>b.onclick=()=>openProductCost(b.dataset.sku))}
const COST_STORAGE_KEY='japanEcProductCostsV119';
let currentCostSku='';
function loadCostStore(){try{return JSON.parse(localStorage.getItem(COST_STORAGE_KEY)||'{}')}catch{return{}}}
function saveCostStore(v){localStorage.setItem(COST_STORAGE_KEY,JSON.stringify(v))}
function costForProduct(p){const saved=loadCostStore()[p?.id]||{};const legacyKg=saved.weightKg===undefined&&saved.weightG!==undefined?num(saved.weightG)/1000:0;return{shippingOrigin:saved.shippingOrigin||'TW',supply:saved.supply??num(p?.supplyPrice),length:saved.length??0,width:saved.width??0,height:saved.height??0,weightKg:saved.weightKg??legacyKg,multiplier:saved.multiplier??'',note:saved.note??''}}
function round2(v){return Math.round((num(v)+Number.EPSILON)*100)/100}
function shippingCalc(c){const realKg=round2(num(c.weightKg)),volKg=round2(num(c.length)*num(c.width)*num(c.height)/(num(calcParams.volDivisor)||6000));if((c.shippingOrigin||'TW')==='JP')return{realKg,volKg,chargeKg:0,method:'日本出貨',freight:0,warehouse:0,logistics:0,isJapan:true};const epacket=realKg<=num(calcParams.epacketMaxKg)&&Math.max(num(c.length),num(c.width),num(c.height))<=num(calcParams.epacketMaxSide)&&(num(c.length)+num(c.width)+num(c.height))<=num(calcParams.epacketMaxSum);const method=epacket?'E小包':'EMS';const chargeKg=method==='E小包'?realKg:Math.max(realKg,volKg);let freight=0;if(method==='E小包'){const step=Math.max(.1,Math.ceil((realKg-1e-9)*10)/10);freight=step<=2?100+Math.max(0,Math.round((step-.1)*10))*20:0}else{const step=Math.max(.1,Math.ceil((chargeKg-1e-9)*10)/10);if(step<=2)freight=289+Math.max(0,Math.round((step-.1)*10))*9;else if(step<=4)freight=540+Math.max(0,Math.round((step-2.1)*10))*10;else freight=730+Math.ceil((step-4)*10-1e-9)*9}const warehouse=num(calcParams.warehouse),logistics=freight+warehouse;return{realKg,volKg,chargeKg:round2(chargeKg),method,freight,warehouse,logistics,isJapan:false}}
function pricingCalc(c,platform='Rakuten',p=null){const ship=shippingCalc(c),mult=num(c.multiplier)||num(calcParams.multiplier),fx=num(calcParams.fxJpyPerTwd),rate=platformFeeRate(platform),formulaPrice=(num(c.supply)*mult+ship.logistics)*fx,revenue=formulaPrice/fx,fee=formulaPrice*rate,vc=vendorCommissionInfo(p),vendorCommissionJpy=formulaPrice*vc.rate,vendorCommissionTwd=fx?vendorCommissionJpy/fx:0,actualVendorPayment=num(c.supply)-vendorCommissionTwd,profit=(formulaPrice-fee)/fx-actualVendorPayment-ship.logistics,margin=revenue?profit/revenue*100:0,target=num(calcParams.targetMargin)/100,den=1-rate+vc.rate-target,targetPrice=den>0?((num(c.supply)+ship.logistics)*fx/den):formulaPrice;return{...ship,mult,formulaPrice,fee,revenue,profit,margin,targetPrice:Math.ceil(targetPrice),vendorCommission:vc,vendorCommissionJpy,vendorCommissionTwd,actualVendorPayment}}
function initCostPricingPage(){const list=$('costProductList');if(list)list.innerHTML=[...state.products.values()].map(p=>`<option value="${esc(p.id)}">${esc(p.managementNumber||'')} ${esc(p.nameZh||'')}</option>`).join('');renderCostList();if(currentCostSku)openProductCost(currentCostSku,false)}
function findCostProductInput(v){const q=normKey(v);return findProduct(v)||findProductByManagementNumber(v)||[...state.products.values()].find(p=>normKey(p.nameZh).includes(q))||null}
function loadCostProductFromInput(){const p=findCostProductInput($('costProductSearch').value);if(!p){$('costProductSummary').textContent='找不到商品，請輸入商品番号／商品管理番号。';return}openProductCost(p.id,false)}
function openProductCost(sku,navigate=true){const p=findProduct(sku);if(!p)return;currentCostSku=p.id;if(navigate)show('costPricing');$('costProductSearch').value=p.id;const c=costForProduct(p);$('costShippingOrigin').value=c.shippingOrigin||'TW';$('costSupply').value=c.supply;$('costLength').value=c.length;$('costWidth').value=c.width;$('costHeight').value=c.height;$('costWeight').value=c.weightKg;$('costMultiplier').value=c.multiplier;$('costNote').value=c.note;$('costProductSummary').innerHTML=`<strong>${esc(p.id)}</strong>　${esc(p.managementNumber||'')}　${esc(p.nameZh||'')}　<span class="muted">${esc(p.projectName||'')}｜廠商：${esc(p.vendorName||'未設定')}</span>`;renderCostPricing()}
function currentCostValues(){return{shippingOrigin:$('costShippingOrigin').value||'TW',supply:num($('costSupply').value),length:num($('costLength').value),width:num($('costWidth').value),height:num($('costHeight').value),weightKg:num($('costWeight').value),multiplier:$('costMultiplier').value===''?'':num($('costMultiplier').value),note:$('costNote').value||''}}
function renderCostPricing(){if(!currentCostSku)return;const p=findProduct(currentCostSku);if(!p)return;const c=currentCostValues(),x=pricingCalc(c,'Rakuten',p);$('costTotal').textContent=twd(x.logistics);$('costFormulaBreakdown').innerHTML=(x.isJapan?[['出貨方式','日本出貨'],['材積重','—（不參與計費）'],['配送方式','日本出貨'],['實際運費計費依據','—'],['運費',twd(0)],['平均進出倉',twd(0)]]:[['出貨方式','台灣出貨'],['實重',x.realKg.toFixed(2)+' kg'],['材積重',x.volKg.toFixed(2)+' kg'],['EMS 計費重',Math.max(x.realKg,x.volKg).toFixed(2)+' kg'],['配送方式',x.method],['實際運費計費依據',x.method==='E小包'?x.realKg.toFixed(2)+' kg 實重':x.chargeKg.toFixed(2)+' kg 計費重'],['運費',twd(x.freight)],['平均進出倉',twd(x.warehouse)]]).map(([n,v])=>`<div><span>${n}</span><strong>${v}</strong></div>`).join('');$('costVendorCommissionSummary').innerHTML=[["廠商",x.vendorCommission.vendor||'—'],["廠商抽成",x.vendorCommission.enabled?(x.vendorCommission.rate*100).toFixed(1)+'%（'+x.vendorCommission.source+'）':'無'],["抽成金額",x.vendorCommission.enabled?yen(x.vendorCommissionJpy)+' / '+twd(x.vendorCommissionTwd):twd(0)],["實際支付廠商貨款",twd(x.actualVendorPayment)]].map(([n,v])=>`<div><span>${n}</span><strong>${v}</strong></div>`).join('');$('costFormulaPrice').textContent=yen(x.formulaPrice);$('costRakutenMargin').textContent=x.margin.toFixed(1)+'%';$('costRakutenMargin').className=x.margin<num(calcParams.targetMargin)?'neg':'pos';$('costTargetPrice').textContent=yen(Math.max(x.formulaPrice,x.targetPrice));$('costPlatformRows').innerHTML=['Rakuten','NETSEA','Shopify','TikTok'].map(platform=>{const z=pricingCalc(c,platform,p),rate=platformFeeRate(platform);return`<tr><td>${platform}</td><td>${yen(z.formulaPrice)}</td><td>${(rate*100).toFixed(1)}%</td><td>${yen(z.fee)}</td><td>${twd(z.revenue)}</td><td>${twd(c.supply)}</td><td>${twd(z.vendorCommissionTwd)}</td><td>${twd(z.actualVendorPayment)}</td><td>${twd(z.logistics)}</td><td>${twd(z.profit)}</td><td class="${z.margin<num(calcParams.targetMargin)?'neg':'pos'}">${z.margin.toFixed(1)}%</td><td>${yen(Math.max(z.formulaPrice,z.targetPrice))}</td></tr>`}).join('')}
function saveCurrentProductCost(){if(!currentCostSku)return;const store=loadCostStore();store[currentCostSku]=currentCostValues();saveCostStore(store);$('costProductSummary').insertAdjacentHTML('beforeend',' <span class="pos">成本已儲存</span>');renderCostPricing();renderCostList()}
function renderCostList(){if(!$('costListRows'))return;const store=loadCostStore(),filter=$('costShippingFilter')?.value||'';$('costListRows').innerHTML=Object.entries(store).filter(([,c])=>!filter||(c.shippingOrigin||'TW')===filter).map(([sku,c])=>{const p=findProduct(sku)||{id:sku},x=pricingCalc(c,'Rakuten',p),jp=(c.shippingOrigin||'TW')==='JP';return`<tr><td><button class="ghost cost-row-open" data-sku="${esc(sku)}">${esc(sku)}</button></td><td>${esc(p.managementNumber||'')}</td><td>${esc(p.nameZh||'')}</td><td>${esc(p.vendorName||'')}</td><td>${jp?'日本出貨':'台灣出貨'}</td><td>${twd(c.supply)}</td><td>${x.vendorCommission.enabled?(x.vendorCommission.rate*100).toFixed(1)+'%':'無'}</td><td>${twd(x.actualVendorPayment)}</td><td>${num(c.length)}</td><td>${num(c.width)}</td><td>${num(c.height)}</td><td>${num(c.weightKg).toFixed(2)}kg</td><td>${jp?'—':x.volKg.toFixed(2)+'kg'}</td><td>${x.method}</td><td>${twd(x.freight)}</td><td>${twd(x.logistics)}</td><td>${yen(x.formulaPrice)}</td><td class="${x.margin<num(calcParams.targetMargin)?'neg':'pos'}">${x.margin.toFixed(1)}%</td><td>${yen(Math.max(x.formulaPrice,x.targetPrice))}</td><td class="cost-note-cell">${esc(c.note||'')}</td></tr>`}).join('');$('costListRows').querySelectorAll('.cost-row-open').forEach(b=>b.onclick=()=>openProductCost(b.dataset.sku,false))}
async function createCostProduct(){if(!['admin','manager'].includes(state.role))return $('newCostStatus').textContent='目前為 Viewer，無法建立商品。';const sku=$('newCostSku').value.trim();if(!sku)return $('newCostStatus').textContent='請輸入商品番号 / SKU';if(findProduct(sku))return $('newCostStatus').textContent='此 SKU 已存在';try{$('newCostStatus').textContent='建立中…';await setDoc(doc(db,'products',sku),{managementNumber:$('newCostManagement').value.trim(),nameZh:$('newCostName').value.trim(),projectName:$('newCostProject').value.trim(),vendorName:$('newCostVendor').value.trim(),supplyPrice:num($('newCostSupply').value),createdAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:false});const store=loadCostStore();store[sku]={shippingOrigin:$('newCostShippingOrigin').value||'TW',supply:num($('newCostSupply').value),length:num($('newCostLength').value),width:num($('newCostWidth').value),height:num($('newCostHeight').value),weightKg:num($('newCostWeight').value),multiplier:'',note:$('newCostNote').value||''};saveCostStore(store);await loadProductMaster();$('costNewModal').classList.add('hidden');openProductCost(sku,false);renderCostList()}catch(e){$('newCostStatus').textContent='建立失敗：'+e.message}}
function pickField(row,names){for(const n of names){if(row[n]!==undefined&&String(row[n]).trim()!=='')return row[n]}return''}
async function importCostFile(file){
  try{
    ...
    if(/\.xlsx?$/i.test(file.name)){
      ...
      rows=await new Promise((res,rej)=>Papa.parse(file,{header:true,skipEmptyLines:'greedy',complete:r=>res(r.data),error:rej}));
    }
    if(!rows.length)throw new Error('檔案沒有可讀取的資料');

    const aliases={
      shippingOrigin:['出貨方式','出貨地','物流方式'],
      sku:['商品番号','SKU','SKU管理番号','商品規格管理編號'],
      mgmt:['商品管理番号','GOT&NETSEA 商品管理番号','商品管理編號'],
      supply:['供應價','商品供應價(TWD)','商品供應價','台幣供貨價 (含稅)NT$','台幣供貨價'],
      length:['長','外箱長(cm)','長度 (cm)','長度'],
      width:['寬','外箱寬(cm)','寬度 (cm)','寬度'],
      height:['高','外箱高(cm)','高度 (cm)','高度'],
      weightKg:['實重','商品實重(kg)','商品重量(kg)','重量(kg)','毛重(kg)'],
      multiplier:['倍率'], note:['備註']
    };
    const headers=Object.keys(rows[0]||{});
    const detected=Object.entries(aliases).filter(([,names])=>names.some(n=>headers.includes(n))).map(([k])=>k);
    const label={shippingOrigin:'出貨方式',sku:'商品番号',mgmt:'商品管理番号',supply:'供應價',length:'長',width:'寬',height:'高',weightKg:'實重(kg)',multiplier:'倍率',note:'備註'};
    if(!detected.includes('sku')||!detected.includes('mgmt'))throw new Error('必須同時包含【商品番号】與【商品管理番号】作為雙重核對欄位');
    const updateFields=detected.filter(k=>!['sku','mgmt'].includes(k));
    if(!updateFields.length)throw new Error('沒有辨識到可更新欄位（出貨方式、供應價、長、寬、高、實重、倍率、備註）');

    const store=loadCostStore(), prepared=[];
    const errors=[]; let blank=0,notFound=0,mismatch=0,invalid=0;
    for(let i=0;i<rows.length;i++){
      const row=rows[i], rowNo=i+2;
      const sku=String(pickField(row,aliases.sku)).trim(),mgmt=String(pickField(row,aliases.mgmt)).trim();
      if(!sku&&!mgmt){blank++;continue}
      if(!sku||!mgmt){errors.push(`第 ${rowNo} 列：商品番号與商品管理番号不可缺一`);mismatch++;continue}
      const bySku=findProduct(sku),byMgmt=findProductByManagementNumber(mgmt);
      if(!bySku||!byMgmt){errors.push(`第 ${rowNo} 列：找不到商品（${sku} / ${mgmt}）`);notFound++;continue}
      if(bySku.id!==byMgmt.id){errors.push(`第 ${rowNo} 列：雙重編號不一致（${sku} / ${mgmt}）`);mismatch++;continue}
      const p=bySku, old=store[p.id]||costForProduct(p), next={...old}; let changed=0,rowBad=false;
      for(const k of updateFields){
        const v=pickField(row,aliases[k]);
        if(v===''||v===null||v===undefined)continue;
        if(k==='note'){next.note=String(v);changed++;continue}if(k==='shippingOrigin'){const t=String(v).trim();if(['日本出貨','日本本地直送','日本直送','日本出貨'].includes(t))next.shippingOrigin='JP';else if(['台灣出貨','台灣','TW','Taiwan'].includes(t))next.shippingOrigin='TW';else{errors.push(`第 ${rowNo} 列：出貨方式「${v}」無法辨識，請填台灣出貨或日本出貨`);invalid++;rowBad=true;break}changed++;continue}
        const n=Number(String(v).replace(/[,，\s]/g,'').replace(/[ＮＴ＄￥¥$]/g,''));
        if(!Number.isFinite(n)||n<0){errors.push(`第 ${rowNo} 列：${label[k]}「${v}」不是有效的非負數`);invalid++;rowBad=true;break}
        next[k]=n;changed++;
      }
      if(rowBad)continue;
      if(!changed){blank++;continue}
      prepared.push({id:p.id,next});
    }
    if(!prepared.length)throw new Error(`沒有可更新資料。找不到 ${notFound}、編號不一致 ${mismatch}、格式錯誤 ${invalid}、空白 ${blank}`);

    const fieldText=detected.map(k=>label[k]).join('、');
    const preview=errors.slice(0,6).join('\n');
    const msg=`已辨識欄位：${fieldText}\n預計更新：${prepared.length} 筆\n找不到商品：${notFound} 筆\n編號不一致：${mismatch} 筆\n格式錯誤：${invalid} 筆\n空白／無變更：${blank} 筆\n\n此功能只更新成本資料，不會修改 Rakuten／NETSEA／Shopify／TikTok 的前台售價。${preview?`\n\n錯誤預覽：\n${preview}`:''}\n\n確定繼續？`;
    if(!confirm(msg)){status.textContent='已取消匯入，未修改任何資料。';return}
    prepared.forEach(x=>store[x.id]=x.next);
    saveCostStore(store);
    status.innerHTML=`完成：<strong>${prepared.length}</strong> 筆更新；找不到 ${notFound}、編號不一致 ${mismatch}、格式錯誤 ${invalid}、空白／無變更 ${blank}。<br>已辨識：${esc(fieldText)}。<strong>平台前台售價未修改。</strong>${errors.length?`<br>錯誤：${esc(errors.slice(0,8).join('；'))}${errors.length>8?'…':''}`:''}`;
    renderCostList();if(currentCostSku)openProductCost(currentCostSku,false);
  }catch(e){$('costImportStatus').textContent='匯入失敗：'+e.message}
}
function renderCalcParams(){if(!$('paramMultiplier'))return;$('paramMultiplier').value=calcParams.multiplier;$('paramFx').value=calcParams.fxJpyPerTwd;$('paramTargetMargin').value=calcParams.targetMargin;$('paramWarehouse').value=calcParams.warehouse;$('paramVolDivisor').value=calcParams.volDivisor;$('paramEpacketMaxKg').value=calcParams.epacketMaxKg;$('paramEpacketMaxSide').value=calcParams.epacketMaxSide;$('paramEpacketMaxSum').value=calcParams.epacketMaxSum;$('paramRakutenFee').value=calcParams.fees.Rakuten;$('paramNetseaFee').value=calcParams.fees.NETSEA;$('paramShopifyFee').value=calcParams.fees.Shopify;$('paramTiktokFee').value=calcParams.fees.TikTok;$('paramVendorDefaultCommission').value=calcParams.vendorDefaultCommission??10;renderVendorCommissionSettings()}
function saveCalcParams(){calcParams={multiplier:num($('paramMultiplier').value)||1,fxJpyPerTwd:num($('paramFx').value)||1,targetMargin:num($('paramTargetMargin').value),warehouse:num($('paramWarehouse').value),volDivisor:num($('paramVolDivisor').value)||6000,epacketMaxKg:num($('paramEpacketMaxKg').value)||2,epacketMaxSide:num($('paramEpacketMaxSide').value)||60,epacketMaxSum:num($('paramEpacketMaxSum').value)||90,vendorDefaultCommission:num($('paramVendorDefaultCommission').value),fees:{Rakuten:num($('paramRakutenFee').value),NETSEA:num($('paramNetseaFee').value),Shopify:num($('paramShopifyFee').value),TikTok:num($('paramTiktokFee').value)}};localStorage.setItem('japanEcCalcParams',JSON.stringify(calcParams));$('calcParamsStatus').textContent='已儲存';renderVendorCommissionSettings();renderMaster();renderPricing();renderSalesStatus();renderCostList();if(currentCostSku)renderCostPricing()}
function renderPricing(){if(!$('pricingRows'))return;const cost=num($('pricingCost').value),price=cost*calcParams.multiplier*calcParams.fxJpyPerTwd;$('pricingRows').innerHTML=['Rakuten','NETSEA','Shopify','TikTok'].map(platform=>{const rate=platformFeeRate(platform),fee=price*rate,revenue=price/calcParams.fxJpyPerTwd,profit=(price-fee)/calcParams.fxJpyPerTwd-cost,margin=revenue?profit/revenue*100:0;return`<tr><td>${platform}</td><td>${(rate*100).toFixed(1)}%</td><td>${calcParams.multiplier.toFixed(2)}</td><td>${yen(price)}</td><td>${yen(fee)}</td><td>${twd(revenue)}</td><td>${twd(profit)}</td><td class="${margin<10?'neg':''}">${margin.toFixed(1)}%</td></tr>`}).join('')}
function renderSalesStatus(){if(!$('salesStatusRows'))return;$('salesStatusRows').innerHTML=['Rakuten','NETSEA','Shopify','TikTok'].map(platform=>{const rows=state.sales.filter(x=>canonicalPlatform(x.platform)===platform),x=summary(rows),fee=x.revenue*platformFeeRate(platform);return`<tr><td>${platform}</td><td>${yen(x.revenue)}</td><td>${fmt(x.quantity)}</td><td>${fmt(x.orders)}</td><td>${calcParams.fees[platform]}%</td><td>${yen(fee)}</td></tr>`}).join('')}
  ...
  Papa.parse(file,{header:true,skipEmptyLines:'greedy',complete:async r=>{
    try{
      const parsed=r.data.map((x,i)=>({
        ...
        tiktokPriceJpy:num(pick(x,['TikTok售價','TikTok日幣售價'])),
        barcode:pick(x,['商品條碼']),
        sourceRow:i+2
      })).filter(x=>x.id);
...
const PARTIAL_PRODUCT_FIELD_MAP=[
  ...
  {keys:['TikTok售價','TikTok日幣售價'],field:'tiktokPriceJpy',type:'number',label:'TikTok售價'},
