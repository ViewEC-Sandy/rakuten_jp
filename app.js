...
$('costLoadBtn').onclick=loadCostProductFromInput;$('costSaveBtn').onclick=saveCurrentProductCost;['costSupply','costDomestic','costInternational','costWarehouse','costPackaging','costOther'].forEach(id=>$(id)?.addEventListener('input',renderCostPricing));
...
const DEFAULT_CALC_PARAMS={multiplier:1.3,fxJpyPerTwd:4.5,fees:{Rakuten:10,NETSEA:4,Shopify:3.4,TikTok:10}};
...
const COST_STORAGE_KEY='japanEcProductCostsV118';
let currentCostSku='';
...
function costForProduct(p){const saved=loadCostStore()[p?.id]||{};return{supply:saved.supply??num(p?.supplyPrice),domestic:saved.domestic??0,international:saved.international??0,warehouse:saved.warehouse??0,packaging:saved.packaging??0,other:saved.other??0}}
function costTotal(c){return ['supply','domestic','international','warehouse','packaging','other'].reduce((a,k)=>a+num(c[k]),0)}
function initCostPricingPage(){const list=$('costProductList');if(list)list.innerHTML=[...state.products.values()].map(p=>`<option value="${esc(p.id)}">${esc(p.managementNumber||'')} ${esc(p.nameZh||'')}</option>`).join('');if(currentCostSku)openProductCost(currentCostSku,false)}
...
function openProductCost(sku,navigate=true){const p=findProduct(sku);if(!p)return;currentCostSku=p.id;if(navigate)show('costPricing');$('costProductSearch').value=p.id;const c=costForProduct(p);$('costSupply').value=c.supply;$('costDomestic').value=c.domestic;$('costInternational').value=c.international;$('costWarehouse').value=c.warehouse;$('costPackaging').value=c.packaging;$('costOther').value=c.other;$('costProductSummary').innerHTML=`<strong>${esc(p.id)}</strong>　${esc(p.managementNumber||'')}　${esc(p.nameZh||'')}　<span class="muted">${esc(p.projectName||'')}</span>`;renderCostPricing()}
function currentCostValues(){return{supply:num($('costSupply').value),domestic:num($('costDomestic').value),international:num($('costInternational').value),warehouse:num($('costWarehouse').value),packaging:num($('costPackaging').value),other:num($('costOther').value)}}
function renderCostPricing(){if(!currentCostSku)return;const p=findProduct(currentCostSku);if(!p)return;const c=currentCostValues(),total=costTotal(c);$('costTotal').textContent=twd(total);$('costFormulaBreakdown').innerHTML=[['商品供應價',c.supply],['日本國內運費',c.domestic],['國際物流',c.international],['倉儲／作業',c.warehouse],['包材',c.packaging],['其他',c.other]].map(([n,v])=>`<div><span>${n}</span><strong>${twd(v)}</strong></div>`).join('')+`<div class="formula-total"><span>總成本</span><strong>${twd(total)}</strong></div>`;$('costPlatformRows').innerHTML=['Rakuten','NETSEA','Shopify','TikTok'].map(platform=>{const price=productPlatformPrice(p,platform),rate=platformFeeRate(platform),fee=price*rate,revenue=calcParams.fxJpyPerTwd?price/calcParams.fxJpyPerTwd:0,profit=revenue-(calcParams.fxJpyPerTwd?fee/calcParams.fxJpyPerTwd:0)-total,margin=revenue?profit/revenue*100:0;return`<tr><td>${platform}</td><td>${yen(price)}</td><td>${(rate*100).toFixed(1)}%</td><td>${yen(fee)}</td><td>${twd(revenue)}</td><td>${twd(total)}</td><td>${twd(profit)}</td><td class="${margin<10?'neg':'pos'}">${margin.toFixed(1)}%</td></tr>`}).join('')}
function saveCurrentProductCost(){if(!currentCostSku)return;const store=loadCostStore();store[currentCostSku]=currentCostValues();saveCostStore(store);$('costProductSummary').insertAdjacentHTML('beforeend',' <span class="pos">成本已儲存</span>');renderCostPricing()}
function renderCalcParams(){if(!$('paramMultiplier'))return;$('paramMultiplier').value=calcParams.multiplier;$('paramFx').value=calcParams.fxJpyPerTwd;$('paramRakutenFee').value=calcParams.fees.Rakuten;$('paramNetseaFee').value=calcParams.fees.NETSEA;$('paramShopifyFee').value=calcParams.fees.Shopify;$('paramTiktokFee').value=calcParams.fees.TikTok}
function saveCalcParams(){calcParams={multiplier:num($('paramMultiplier').value)||1,fxJpyPerTwd:num($('paramFx').value)||1,fees:{Rakuten:num($('paramRakutenFee').value),NETSEA:num($('paramNetseaFee').value),Shopify:num($('paramShopifyFee').value),TikTok:num($('paramTiktokFee').value)}};localStorage.setItem('japanEcCalcParams',JSON.stringify(calcParams));$('calcParamsStatus').textContent='已儲存';renderMaster();renderPricing();renderSalesStatus()}
